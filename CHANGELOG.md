...
### Added
- **Shell completion + ID-aware completers (P7.T1 / M1).** Adds a new `notebooklm completion <bash|zsh|fish>` top-level command that prints the Click-generated completion script for the chosen shell on stdout — pipe it to a file your shell sources at startup (`notebooklm completion zsh > ~/.zfunc/_notebooklm`, install snippets for all three shells in the per-command `--help` and in `docs/cli-reference.md`). The command is intentionally print-only so the user controls the install path; auto-installing into shell rc files would be too magical and would hide the install location from users who care. Once the script is sourced, Click's `_NOTEBOOKLM_COMPLETE` env-var protocol kicks in automatically with no further glue. On top of that, the canonical `-n/--notebook`, `-s/--source`, and `-a/--artifact` options now ship with `shell_complete` callbacks that emit live IDs from the active profile: `notebooklm <cmd> -n <TAB>` lists notebook IDs (filtered by the prefix already typed); `-s <TAB>` and `-a <TAB>` resolve the active notebook (flag > `NOTEBOOKLM_NOTEBOOK` env > persisted `notebooklm use` context) and list its sources / artifacts. The wiring is centralized in `cli/options.py:notebook_option/source_option/artifact_option` so every command using those decorators gets the completer at once; the inline `-a/--artifact` declarations in `cli/download.py` (9 sites) and `cli/generate.py:revise-slide`, plus the `--source/-s` filter options in `cli/chat.py:ask` and `cli/generate.py` (8 sites — `multiple=True`, so TAB-cycling individual IDs works) were also wired so no surface is left behind. Both completers cap at 50 results to keep the shell snappy on large profiles. Critically, every callback is **best-effort**: any exception (missing auth, offline, rate-limited API, transient JSON decode error) degrades silently to `[]` so a fresh shell never gets a Python traceback printed mid-completion. The completion command is binned in the Session section of `notebooklm --help` (see `cli/grouped.py`) so it's discoverable next to `login`, `auth`, and `doctor`. Regression suite: `tests/unit/cli/test_completion.py` (24 tests covering script-emission for each shell, invalid-shell UsageError exit-2, missing-arg UsageError, `--help` discoverability, install-snippet content, `_complete_notebooks` filtering / auth-failure / network-error / 50-row cap, `_resolve_notebook_for_completion` flag-vs-env-vs-context precedence + blank-env fallthrough, `_complete_sources` / `_complete_artifacts` empty-without-notebook + filter-by-prefix + swallow-listing-error, and decorator-wiring assertions for all three options).
- **`NOTEBOOKLM_NOTEBOOK` env var + global `--quiet` flag (P7.T3 / M4).** Two additive global affordances for shell / CI / agent automation, plus one consolidated documentation pass. (1) `NOTEBOOKLM_NOTEBOOK=<id> notebooklm ask "..."` now works without `-n/--notebook` and without first running `notebooklm use <id>`; the env var is wired natively on the shared `cli/options.py:notebook_option` decorator (`envvar="NOTEBOOKLM_NOTEBOOK", show_envvar=True`) so every subcommand exposing `-n` picks it up at once with no per-command boilerplate AND so the binding shows up in `--help` automatically. The same env var is re-checked inside `cli/helpers.require_notebook` so direct programmatic callers honor the same precedence ladder. Resolution order: `-n/--notebook` flag > `NOTEBOOKLM_NOTEBOOK` env > active context (from `notebooklm use`) > error. Empty / whitespace-only env values are treated as unset so a bare `export NOTEBOOKLM_NOTEBOOK=` does not block the next fallback. (2) `notebooklm --quiet <subcommand>` raises the `notebooklm` package logger floor to ERROR for the duration of one invocation, so cron / CI logs stay clean while real failures still surface. It is mutually exclusive with `-v/-vv` — combining the two raises `UsageError: --quiet and -v are mutually exclusive.` (exit 2) because the resolved log levels conflict (ERROR vs INFO/DEBUG). The new flag is purely additive: omitting it leaves the existing `NOTEBOOKLM_LOG_LEVEL` / `-v/-vv` precedence intact. (3) A new "Env vars and precedence" section in `docs/configuration.md` lists every `NOTEBOOKLM_*` env var (`NOTEBOOKLM_PROFILE`, `NOTEBOOKLM_AUTH_JSON`, `NOTEBOOKLM_HOME`, `NOTEBOOKLM_NOTEBOOK`, `NOTEBOOKLM_HL`, `NOTEBOOKLM_LOG_LEVEL`, `NOTEBOOKLM_DEBUG_RPC`, `NOTEBOOKLM_RPC_OVERRIDES`, `NOTEBOOKLM_QUIET_DEPRECATIONS`, `NOTEBOOKLM_STRICT_DECODE`) with its purpose, full resolution order, and the canonical resolver location, so the precedence rule for any variable can be audited from one place. The existing `auth refresh --quiet` subcommand-scoped flag is unaffected (Click parses the closer scope first, so `notebooklm auth refresh --quiet` continues to hit the subcommand flag exactly as before). Regression suite: `tests/unit/cli/test_root_group.py::TestQuietFlag` (5 tests covering log-level-set, mutex-with-`-v`, mutex-with-`-vv`, baseline-no-mutation, and `--help` discoverability) + `TestNotebookOptionEnvVar::test_notebook_option_declares_envvar_binding`; `tests/unit/cli/test_helpers.py::TestRequireNotebook` (4 new tests covering env-var-resolution, flag-overrides-env, env-overrides-context, blank-env-falls-through, and value-stripping).
- **Unix `-` (stdin) convention on `ask`, `note create`, `source add`, and `--prompt-file` (P7.T2 / M3).** Four CLI surfaces now accept the canonical `-` placeholder for "read from stdin", matching the Unix tradition (`cat`, `tee`, `jq`, etc.) so pipelines compose naturally without resorting to process substitution or temp files: (1) `notebooklm ask -` reads the question from stdin (`echo "what is X?" | notebooklm ask -`); (2) `notebooklm ask --prompt-file -` reads the prompt from stdin (replaces `click.Path(exists=True)` with a custom `_PromptFilePath` param type that allows the literal `-` while preserving the existence + dir-okay checks for real paths so a typo still fails fast at parse time); (3) `notebooklm note create` gains a new `--content` flag (mutually exclusive with the positional `CONTENT` argument) and both forms accept `-` (`cat notes.md | notebooklm note create --content -` or the positional `notebooklm note create -`); (4) `notebooklm source add -` reads inline text from stdin and forces the text-source path, intercepted BEFORE the URL / file / path-shaped auto-detection branches so a single dash never falls into the path-shaped warning ("'-' looks like a path...") and an explicit `--type file -` does not try to open a file literally named `-`. A new shared `helpers.read_stdin_text()` utility centralizes the stdin-read + UTF-8-decode + whitespace-strip path so all four call sites stay in lockstep, and uses `click.get_text_stream("stdin").read()` so `CliRunner.invoke(input=...)` in tests is honored without monkey-patching `sys.stdin`. Default behavior on every command is unchanged when `-` is not passed: literal arguments and real file paths flow through the original code paths exactly as before. Regression suite: `tests/unit/cli/test_chat.py::TestAskStdinDash` (3 tests), `tests/unit/cli/test_note.py::TestNoteCreateStdinDash` (4 tests covering `--content -`, positional `-`, literal-value-unchanged, and the positional+`--content` UsageError conflict), `tests/unit/cli/test_source.py::TestSourceAddStdinDash` (3 tests including a `--title` interop test and a literal-text regression).
