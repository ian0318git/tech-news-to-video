...
FETCH_DEADLINE = 45.0  # 執行緒護欄總期限(_http.bounded_get);必須 > REQUEST_TIMEOUT
...
def fetch_rss(channel: dict) -> str:
    ...
    try:
        # 執行緒護欄:httpx timeout 管不到 DNS 解析卡死(2026-09-08 教訓)—
        # 逾時快速失敗,交下一輪 catch-up 重試
    ...
    except _http.GuardTimeoutError:
        fail(
            logger,
